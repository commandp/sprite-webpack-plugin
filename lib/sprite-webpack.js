...
module.exports = {
  ...
  createImage: function(opt, createImageCallback) { //use options
    if (!Util.checkSource(opt.source)) { return }
    if (!opt.info) { opt.info = updateLayer(opt); } //use options
    function create(meta, createCallback) {
      async.waterfall([
        function(next) {
          lwip.create (
            meta.info.width + 200,
    ...
    if (opt.multiFolders) {
      ...
      async.each(imagesInfo, function(imgInfo, imageCallback) {
        Util.checkDir(imgInfo.imgPath);
        create(imgInfo, imageCallback);
      }, createImageCallback);
      return;
    }
    Util.checkDir(opt.imgPath);
    create(opt, createImageCallback);
  },
